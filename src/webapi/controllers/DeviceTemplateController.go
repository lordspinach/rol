...
func NewDeviceTemplateController(config *domain.AppConfig, log *logrus.Logger) (*DeviceTemplateController, error) {
	service, err := services.NewDeviceTemplateService(config, log)
	if err != nil {
		return nil, fmt.Errorf("error getting device template service: %s", err.Error())
	}
