...
// @Success	200		{object}	dtos.PaginatedItemsDto[dtos.TFTPConfigDto]
...
func (t *TFTPServerGinController) GetList(ctx *gin.Context) {
	...
	paginatedList, err := t.service.GetList(ctx, search, orderBy, orderDirection, int(pageInt64), int(pageSizeInt64))
...
// @Success	200		{object}	dtos.TFTPConfigDto
...
func (t *TFTPServerGinController) GetByID(ctx *gin.Context) {
	...
	dto, err := t.service.GetByID(ctx, id)
...
// @Accept	json
// @Produce	json
// @Param	request	body		dtos.TFTPConfigCreateDto	true	"TFTP server fields"
// @Success	200		{object}	dtos.TFTPConfigDto
// @Failure	400		{object}	dtos.ValidationErrorDto
// @Failure	500		"Internal Server Error"
// @router /tftp/ [post]
func (t *TFTPServerGinController) Create(ctx *gin.Context) {
	reqDto, err := getRequestDtoAndRestoreBody[dtos.TFTPConfigCreateDto](ctx)
	if err != nil {
		abortWithStatusByErrorType(ctx, err)
		return
	}

	dto, err := t.service.Create(ctx, reqDto)
	handleWithData(ctx, err, dto)
}
...
// @Produce	json
// @param	id		path		string		true	"TFTP server ID"
// @Param	request	body		dtos.TFTPConfigUpdateDto true "TFTP server fields"
// @Success	200		{object}	dtos.TFTPConfigDto
// @Failure	400		{object}	dtos.ValidationErrorDto
// @Failure	404		"Not Found"
// @Failure	500		"Internal Server Error"
...
func (t *TFTPServerGinController) Update(ctx *gin.Context) {
	reqDto, err := getRequestDtoAndRestoreBody[dtos.TFTPConfigUpdateDto](ctx)
	if err != nil {
		abortWithStatusByErrorType(ctx, err)
		return
	}
	...
	dto, err := t.service.Update(ctx, reqDto, id)
	handleWithData(ctx, err, dto)
}
...
func (t *TFTPServerGinController) GetPaths(ctx *gin.Context) {
	...
	paginatedList, err := t.service.GetPaths(ctx, serverID, orderBy, orderDirection, int(pageInt64), int(pageSizeInt64))
	handleWithData(ctx, err, paginatedList)
}
